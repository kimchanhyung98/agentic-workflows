...
A2A는 "에이전트가 다른 에이전트에게 일을 위임하고, 진행 상태와 결과를 표준 방식으로 받는 것"에 집중함. 또한 장기 실행을 전제로 streaming(SSE)과 push notification을 함께
다룸.
...
- 에이전트의 공개 메타데이터 문서로, `/.well-known/agent-card.json` 경로에 게시됨.
- 기능(capabilities), `supportedInterfaces`, 인증 요구사항, 스킬, Extensions를 제공함.
- 클라이언트는 호출 전에 Agent Card를 확인해 "이 에이전트가 무엇을, 어떤 방식으로 처리 가능한지"를 판단함.
- JWS(RFC 7515) + JSON Canonicalization(RFC 8785) 기반 디지털 서명을 지원함.

### 2.2 Task

- A2A 상호작용의 중심 단위임.
- 단순 request/response보다 긴 수명의 작업 객체로 관리됨.
- 상태 전이를 갖고 `GetTask`, `ListTasks` 등으로 조회 가능함.
- 8가지 상태: `SUBMITTED`, `WORKING`, `COMPLETED`, `FAILED`, `CANCELED`, `INPUT_REQUIRED`, `REJECTED`, `AUTH_REQUIRED`

### 2.3 Message

- Task 안에서 오가는 대화/명령 단위임.
- 사용자 질의, 시스템 지시, 중간 질문 등 상호작용 히스토리를 담음.

### 2.4 Part

- Message를 구성하는 세부 조각임.
- 멀티모달 확장을 고려한 구조로, 텍스트 외에도 파일 URL, 바이너리(raw), 구조화 데이터(JSON) 같은 표현을 담을 수 있음.

### 2.5 Artifact

- Task 수행 결과로 생성되는 산출물임.
- 최종 답변뿐 아니라 중간/최종 결과물을 구조적으로 전달하는 데 사용됨.
...
이 계층 분리 덕분에 구현체는 "발견", "호출", "장기 실행 통지"를 독립적으로 설계할 수 있음.
...
- 기본 동기 호출임.
- 요청을 보내고 표준 응답을 수신함.
- 짧거나 즉시 완료 가능한 작업에 적합함.

### 4.2 `SendStreamingMessage`

- SSE 기반 스트리밍 호출임.
- Task 진행 이벤트/중간 결과를 순차적으로 받음.
- 장기 실행 또는 진행 가시성이 중요한 시나리오에 적합함.

### 4.3 `GetTask`

- 특정 Task의 최신 상태/결과를 조회함.
- 연결이 끊겼거나 나중에 결과를 재조회할 때 사용함.

### 4.4 `ListTasks`

- Task 목록을 필터링/조회함. 커서 기반 페이지네이션을 지원함.
- 운영/모니터링 관점에서 유용함.

### 4.5 `CancelTask`

- 장기 작업 중단을 요청함.
- 자원 보호, 사용자 취소 UX, SLA 관리에 필요함.

### 4.6 `SubscribeToTask` / Push Notification Config

- 장기 작업 결과를 웹훅 등으로 비동기 수신하기 위한 설정임.
- 클라이언트가 상시 연결을 유지하지 않아도 완료/실패 이벤트를 받을 수 있음.
...
실무에서는 B + D 조합(초기 실시간 가시성 + 최종 비동기 통지)이 자주 사용됨.

---

## 6. 보안/엔터프라이즈 고려사항

엔터프라이즈 환경에서는 다음을 함께 고려해야 함. (상세 분석은 [보안 분석](/a2a/03-security.md) 참고)
...
> 구현 상세는 [구현 가이드](/a2a/04-implementation.md)를 참고.

---

## 9. 트레이드오프

- **표준 상호운용성 vs 구현 복잡도**: 공통 인터페이스 이점이 크지만 Task/스트리밍/푸시까지 구현 범위가 넓어짐.
- **실시간성 vs 운영비용**: streaming은 사용자 경험이 좋지만 연결/이벤트 관리 비용이 증가함.
- **유연성 vs 거버넌스**: 에이전트 간 자유로운 위임은 강점이나, 엔터프라이즈에서는 정책/감사 통제가 필수임.
- **분리된 책임 vs 디버깅 난이도**: A2A(협업) + MCP(도구) 조합은 구조적으로 우수하지만 문제 추적 경로가 길어질 수 있음.

결론적으로 A2A는 멀티에이전트 협업의 "계약 계층"으로 유용하며, MCP와 결합할 때 실제 운영 가치가 크게 올라감.
...
> 스펙 상세는 [프로토콜 스펙 상세](/a2a/02-specification.md)를 참고.
