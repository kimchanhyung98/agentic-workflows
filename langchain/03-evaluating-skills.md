...
Skill의 품질을 체계적으로 측정하고 개선하기 위한 평가 방법론임.
Skills는 코딩 에이전트에 동적으로 로드되는 프롬프트이므로, 프롬프트와 마찬가지로 체계적인 테스트가 필요함.

> **핵심 아이디어**: Skills는 프롬프트처럼 동작하며, 프롬프트처럼 테스트해야 함. 실제 실패 사례를 기반으로 태스크를 정의하고, Skills 유무에 따른 성능 차이를 정량적으로 측정함.

---

## 평가가 필요한 이유

에이전트 시스템은 비결정적(non-deterministic)임. Skills를 추가하거나 수정하면 예측하기 어려운 방식으로 에이전트 행동이 변할 수 있음.
...
실제 코딩 에이전트의 **실패 사례를 기반**으로 태스크를 정의함.
...
태스크 수행에 필요한 도메인 지침을 SKILL.md로 작성함.

### 3단계: 베이스라인 테스트

Skills 없이 코딩 에이전트에게 동일한 태스크를 수행시키고 결과를 기록함.

### 4단계: 성능 비교

Skills를 적용한 후 동일 태스크를 수행시키고 결과를 비교함.
...
Skill이 **적절할 때만 호출**되는지 검증하는 것이 중요함:
...
LangSmith의 pytest 통합과 트레이싱 기능을 활용하여 코딩 에이전트의 전체 실행 경로를 관찰함:
...
감으로 태스크를 만들지 않음. 코딩 에이전트가 **실제로 실패한 사례**를 관찰하고, 그 실패를 재현하는 태스크를 작성함.

### 2. 가장 단순한 테스트 케이스

실패를 재현할 수 있는 **최소한의 테스트 케이스**를 작성함. 복잡한 태스크는 실패 원인을 파악하기 어려움.

### 3. 오픈엔디드 출력 제약

코딩 에이전트의 출력이 자유로우면 평가가 어려움. 버그 수정처럼 **출력이 제약된 태스크**가 검증하기 쉬움.

### 4. 반복적 개선

평가 → Skill 수정 → 재평가 사이클을 반복함. 관찰 가능성(Observability)을 통해 점진적으로 개선함.
...
Skill 변경 시 자동으로 평가를 실행하는 CI 파이프라인을 구성할 수 있음:
