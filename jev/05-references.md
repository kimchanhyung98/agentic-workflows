...
외부 출처 링크는 TypeSafe와 비교 대상 프로젝트의 공식 문서, 직접 확인한 TypeSafe 콘솔만 사용합니다. 이 디렉토리 안의 문서 링크는 설명을 연결하는 용도이며, 다른 프로젝트의 문서나 로컬 조사 자료를 전제로 하지 않습니다.
...
역할별 차이와 적용 한계는 [비교 문서](04-comparison.md)에 정리했습니다. 이 프로젝트들을 같은 데이터셋으로 실행해 성능을 비교한 것은 아닙니다.
...
2026-09-22에 로그인된 [TypeSafe Playground 콘솔](https://console.typesafe.ai/playground)에서 가상 고객 문장 10개를 총 14회 실행하고 화면의 JSON 결과를 직접 확인했습니다. 계정 정보·API 키·고객 개인정보는 문서에 기록하지 않았습니다.

후속으로 새 합성 사례 100개를 각각 10회 직접 HTTP 호출했습니다. 정상 응답 1,000개, 529 실패를 포함한 시도 1,001회입니다. 이는 콘솔 실행이 아닌 API 실험이며, 이전 Playground 결과와 합치지 않고 별도 분석했습니다.
...
실험용 코드·테스트·입력 파일·원시 응답·분석 JSON은 저장소에서 제거했습니다. 위 보고서는 당시 관측과 집계 결과를 정리한 문서이며, 현재 저장소만으로 원문 대조나 재집계는 할 수 없습니다.
