...
이 문서는 Stripe Minions의 내부 시스템 설계를 상세히 다룸.
격리된 실행 환경(Devbox), Agent Core의 LLM과 결정론적 게이트 결합 전략, 3단계 피드백 루프, 그리고 최종 산출물인 PR 생성까지의 과정을 설명함.

---

## Layer 3: Devbox — Isolated VM

에이전트는 격리된 VM(Devbox) 안에서 실행됨. 인간 엔지니어가 사용하는 것과 **동일한 개발 환경**임.
...
인터넷과 프로덕션 접근이 구조적으로 차단되어 있기 때문에, **인간의 승인 없이도 안전하게 실행**할 수 있음.
샌드박스의 격리 수준 자체가 권한 모델이 되는 것임.
...
Minions의 Agent Core는 Block(구 Square)이 개발한 오픈소스 코딩 에이전트 **Goose**의 포크 버전임.

### 실행 파이프라인

Agent Core는 **LLM 창의적 단계**와 **결정론적 게이트**를 교차 배치함:
...
LLM의 창의성과 결정론적 코드의 신뢰성 사이에서 균형을 잡기 위해, 에이전트가 절대 우회할 수 없는 규칙이 존재함:
...
이러한 가드레일은 에이전트가 "지름길"을 택하는 것을 **구조적으로** 방지함.
LLM에게 "린터를 실행하세요"라고 프롬프트하는 것이 아니라, 린터 게이트를 파이프라인에 하드코딩한 것임.

---

## Layer 5: Three-Tier Feedback Loop

코드 변경 후 3단계 피드백 루프가 동작함:
...
2번 시도해도 수정할 수 없으면 인간에게 돌려줌. 무한 재시도로 토큰을 낭비하지 않음.

---

## Layer 6: Output — PR Ready for Review

모든 레이어를 통과한 결과물은 리뷰 준비가 완료된 PR임.
...
Minions의 핵심 설계 원칙은 인간 엔지니어와 AI 에이전트가 **동일한 도구**를 사용한다는 것임:
...
에이전트 전용 도구를 별도로 만드는 것이 아니라, 인간이 이미 사용하는 인프라를 그대로 활용함.
