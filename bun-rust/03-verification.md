...
이 문서는 `bun-rust` 분석에서 사용한 근거의 종류와 각 근거로 말할 수 있는 범위를 정리함. 목적은 별도의 구현이나 검사 산출물을 검증하는 것이 아니라, 관찰 사실과 역설계 모델의 경계를 분명히 하는 것임.
...
`확인`은 공개 자료에서 직접 읽을 수 있다는 뜻임. Bun 팀이 자체적으로 집계한 수치는 `공식 집계`, Bun 밖의 제한된 환경에서 나온 결과는 `외부 관찰`, 공개된 동작을 연결해 만든 구조는 `역설계`로 구분함.
...
병합 commit `23427dbc12fdcff30c23a96a3d6a66d62fdc091d`의 tree SHA는 `d0c85750ebaae0ebf4c797435b7362ea26c7e7df`임. tree 목록은 `truncated=false`임. 분석에서 인용한 경로는 다음 아홉 개임.
...
같은 tree의 `.claude/workflows/*.workflow.js`는 53개임. 이는 공식 회고의 “약 50개 dynamic workflow”와 규모가 어긋나지 않지만, 파일 수를 동시 실행 수나 실제 호출 횟수로 읽으면 안 됨.
...
이 수치는 공식 글과 일치한다는 의미에서 확인할 수 있지만, Bun 내부에서 수치를 생성한 원장까지 독립적으로 검증한 것은 아님. 공식 회고는 Bun 작성자의 자기 보고이며, Bun이 Anthropic에 인수된 뒤 출시 전 Claude Fable 5를 사용한 사례를 설명한다는 이해관계도 함께 고려해야 함.

## 4. unsafe 감사

[Bun unsafe audit](https://bun.com/bun-unsafe-audit)는 commit `3eb0fda021`을 대상으로 2026-05-21에 작성됐고, 페이지 자체가 AI 생성물임을 명시함. 이 감사는 `unsafe` 13,365개를 block 단위로 세고 약 9,300개는 safe code로 바꿀 수 있으며 약 4,000개는 남는다고 분류함.

감사는 safe Rust에서 undefined behavior에 도달할 수 있는 unsound 함수 다섯 개도 별도로 지목함. 다만 상위 네 패턴만 지점별로 측정하고 나머지는 확인된 수치 위의 추정이라고 밝힘. 공식 회고의 약 13,000개 `unsafe` 키워드와 감사의 13,365 block은 단위와 commit이 달라 직접 비교할 수 없음.

## 5. 외부 운영 관찰

[Prisma Compute 보고서](https://www.prisma.io/blog/bun-rust-rewrite-prisma-compute)는 메모리 비교와 Prisma 자체 운영 관찰을 구분해 읽어야 함.

- 메모리 비교는 issue #29083의 후속 실행을 인용함. 안정판 `1.3.14+0d9b296af`는 약 96회에 900MiB를 넘었고, canary `1.3.14-canary.1+172afa532`는 4,096회 실행 동안 sampled RSS 최대 약 118MiB였음.
- Prisma가 직접 관찰한 결과는 scale-to-zero 복귀 뒤 dead idle connection 때문에 SQL pool이 멈추던 문제가 Rust canary에서는 나타나지 않았다는 것임.

보고서도 모든 pool 실패가 해결됐다고 주장하지 않음. 이 결과는 Prisma Compute의 당시 workload와 명시된 build에만 적용함.

## 6. 2차 자료

[GeekNews 글](https://news.hada.io/topic?id=31263)은 공식 회고의 한국어 색인과 요약으로 유용하지만 독립 검증 자료는 아님. Hacker News·Lobsters 토론과 Zig의 no-AI 정책에 대한 편집 서술도 포함함.

[WikiDocs 글](https://wikidocs.net/blog/@jaehong/13487/)의 `6~9일`, `99.8%`, `약 96만 줄 Zig`, `unsafe 13,044개`는 2026-05-15 무렵의 중간 스냅샷임. 공식 최종 회고와 집계 시점·분모·단위가 달라 이 문서의 사실 기준으로 사용하지 않음.

## 7. 역설계 모델의 한계

`02-implementation-flow.md`는 공개 workflow를 queue, 작업 셀, 통합 장벽, 재검증 라운드로 설명함. 다음 요소는 공개 자료에서 직접 확인되지 않음.
...
따라서 역설계 모델은 공개된 동작 사이의 관계를 설명하는 도구임. 그 모델이 논리적으로 자연스럽다는 사실과 Bun이 실제로 같은 내부 구현을 사용했다는 주장은 서로 다름.
