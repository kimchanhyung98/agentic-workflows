...
Andrew Ng이 제시한 4가지 핵심 Agentic Workflow 디자인 패턴을 정리함.
이 패턴들은 AI 에이전트의 성능을 크게 향상시키는 기본 빌딩 블록으로, 단독 또는 조합하여 활용할 수 있음.
...
Reflection은 **에이전트가 자신의 출력을 스스로 검토하고, 오류나 개선점을 찾아 수정하는** 패턴임.

사람이 글을 작성한 뒤 퇴고(推敲)하는 과정과 유사함. 한 번의 시도로 완벽한 결과를 내기 어려운 작업에서, 반복적인 자기 검토를 통해 품질을 향상시킴.
...
Tool Use는 **에이전트가 외부 도구(API, 데이터베이스, 코드 인터프리터 등)를 호출하여 LLM만으로는 수행할 수 없는 작업을 처리하는** 패턴임.

LLM은 학습 데이터의 시점까지의 지식만 보유하며, 정확한 수학 계산이나 실시간 데이터 접근이 어려움. 도구를 통해 이러한 한계를 극복함.
...
Planning은 **에이전트가 복잡한 목표를 달성하기 위해 작업을 하위 단계로 분해하고, 실행 순서를 결정하는** 패턴임.

사람이 프로젝트를 관리할 때 작업 목록을 만들고 우선순위를 정하는 것과 유사함. 에이전트는 전체 작업을 조망하며 효율적인 실행 경로를 설계함.
...
| **마케팅 캠페인**  | 타깃 분석 → 콘텐츠 기획 → 제작 → 배포 → 성과 측정  |
...
Multi-Agent Collaboration은 **서로 다른 역할과 전문성을 가진 여러 에이전트가 협력하여 복잡한 작업을 수행하는** 패턴임.

실제 팀이 역할을 분담하여 프로젝트를 진행하듯, 각 에이전트가 고유한 전문 분야를 담당하고, 서로 소통하며 최종 결과를 만들어냄.
...
| **Agents SDK** | OpenAI    | 경량 멀티 에이전트 오케스트레이션 (Swarm의 후속 프로덕션 버전) |
...
실제 시스템에서는 단일 패턴보다 **여러 패턴을 조합**하여 사용하는 경우가 많음.
