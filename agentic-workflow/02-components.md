...
AI 에이전트는 여러 핵심 모듈이 유기적으로 결합되어 작동하는 시스템임.
이 문서에서는 에이전트를 구성하는 핵심 요소와 각 요소의 역할을 살펴봄.
...
에이전트의 **중앙 추론 엔진**으로, 대형 언어 모델(LLM)이 이 역할을 수행함.
...
복잡한 작업을 **관리 가능한 하위 작업으로 분해**하고 실행 순서를 결정함.
...
에이전트가 **과거 경험과 컨텍스트를 저장하고 활용**하는 시스템임.
...
에이전트가 **외부 환경과 상호작용**하기 위해 사용하는 기능들임.

LLM 단독으로는 최신 정보를 알지 못하거나, 정확한 계산을 수행하지 못하는 한계가 있음. 도구를 통해 이러한 한계를 보완함.
...
에이전트의 추론 결과를 **실제로 실행에 옮기는** 구성 요소임.

행동은 두뇌의 의사결정에 따라 도구를 호출하거나, 사용자에게 응답을 생성하거나, 환경의 상태를 변경함.
...
아래 다이어그램은 에이전트가 작업을 수행하는 전체 흐름을 보여줌.
...
에이전트의 자율성과 복잡도에 따라 다양한 유형으로 분류할 수 있음.
