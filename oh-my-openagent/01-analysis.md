...
공식 문서에서는 이를 "계획과 실행의 분리(Planning/Execution Separation)"로 설명하며, Prometheus(계획)와 Atlas(실행)를 중심으로 구성함.

---

## 2. 플러그인 초기화 파이프라인

OpenCode CLI가 시작되면 `OhMyOpenCodePlugin`이 초기화됨. 이 과정은 5단계로 분리됨.

### 2.1 설정 로드 및 병합

`initConfigContext` + `loadPluginConfig`가 **3단계 설정 병합**을 수행함.

1. **Project 설정** — 프로젝트 루트의 로컬 설정 (최우선)
2. **User 설정** — 사용자 전역 설정
3. **Defaults** — 플러그인 기본값

이 병합 순서로 프로젝트별 커스터마이징과 사용자 기본값이 자연스럽게 공존함.

### 2.2 매니저 생성

설정 병합 후 4개의 매니저가 생성됨.
...
매니저 생성 후 **기본 설정 기준 26개 도구**가 등록됨. 실제 등록 수는 `task_system`, `hashline_edit` 등 옵션 활성화 여부에 따라 달라짐.

### 2.4 훅 생성

도구 등록 후 **기본 설정 기준 46개 훅**이 생성됨. `isHookEnabled` 설정에 따라 실제 활성화되는 훅 수는 달라질 수 있음.
...
모든 훅과 도구가 등록되면 플러그인 인터페이스가 반환되고 사용자 입력 대기 상태로 진입함.

---

## 3. 메시지 처리 파이프라인

사용자 메시지 입력부터 API 호출, 응답 반환까지 3단계로 처리됨.

### 3.1 1단계: 사전 개입 (chat.message)

`chat.message` 핸들러는 내부적으로 10단계 파이프라인으로 동작하지만, 여기서는 그중 핵심 8단계만을 아래 순서로 정리함.
...
LLM이 도구 호출을 반환하면 Before → Execute → After 3단계로 처리됨.
...
`writeExistingFileGuard`는 파일 덮어쓰기 사고를 방지하는 안전장치임. `commentChecker`는 AI가 생성하는 불필요한 코멘트를 사전에 차단함.

### 4.2 도구 실행

실제 실행되는 도구는 4개 카테고리로 분류됨.
...
After 훅의 핵심은 **자동 복구**임. `editErrorRecovery`와 `delegateTaskRetry`가 일시적 실패를 자동으로 재시도하며, `jsonErrorRecovery`가 파싱 오류를
보정함. 결과는 LLM의 다음 단계로 반환됨.
...
시스템은 메인 오케스트레이터 **Sisyphus**를 정점으로 9개 전문 에이전트를 위임 체계로 운영함.
...
Sisyphus가 태스크 성격에 따라 적절한 에이전트에 위임하며, 모든 위임은 Sisyphus → 에이전트 단방향임.

**Atlas**(🗺️, claude-sonnet-4-6)는 Todo 오케스트레이터로, Sisyphus에게 Boulder 강제 실행을 지시하는 특수 위치에 있음.

### 5.2 도구 제한 정책

에이전트별로 사용 가능한 도구가 엄격히 제한됨.
...
읽기 전용 에이전트(Oracle, Librarian, Explore)는 코드를 변경할 수 없고, Looker는 파일 읽기만 가능함. 이 제한은 에이전트 간 책임 분리와 안전성을 보장함.

### 5.3 에이전트 간 역할 분담 패턴

일반적인 작업 흐름에서 에이전트 간 역할 분담은 다음과 같음:
...
각 에이전트는 주 모델 실패 시 대체 모델로 자동 전환됨.
...
API 에러 발생 시 3단계로 분류됨:

1. **재시도 가능** — 같은 모델로 재시도 (일시적 오류, rate limit 등)
2. **폴백 필요** — 체인의 다음 모델로 전환 (모델 다운, 지속적 실패)
3. **치명적** — 실패 보고 후 중단 (인증 오류 등 복구 불가)

재시도 실패 시 자동으로 폴백 체인이 진행됨. 다양한 프로바이더(Anthropic, OpenAI, Google, xAI 등)를 섞어 단일 프로바이더 장애에 대한 복원력을 확보함.

---

## 7. Boulder 연속 실행 메커니즘

`todo-continuation-enforcer`로 구현된 Boulder는 미완료 TODO가 남아 있으면 LLM 작업을 자동으로 재개하는 메커니즘임.
...
대기 후 다시 복구(Recovery)를 시도함. 사용자 취소 또는 에이전트 에러 시 즉시 중단됨.

---

## 8. Atlas 마스터 오케스트레이터

Atlas는 `session.idle` 이벤트를 받아 7단계 의사결정 게이트를 통해 다음 행동을 결정함.
...
Atlas는 Boulder의 상위 제어자로서, Boulder가 "어떻게 연속 실행할지"를 담당한다면 Atlas는 "연속 실행을 할지 말지"를 결정함.
...
`task()` 도구 호출 시 `LaunchInput`(description, prompt, category, subagent_type)을 구성하고, 3단계 검증을 거침.

1. **스폰 예산 확인** — 초과 시 스폰 거부
2. **중첩 깊이 확인** — 초과 시 스폰 거부
3. **동시성 한도 확인** — 모델당 5개 제한, 초과 시 대기열 진입

검증 통과 후 서브에이전트가 스폰되어 **격리된 백그라운드 세션**에서 독립 실행됨.

### 9.2 실행 및 안전장치

백그라운드 세션은 자체 컨텍스트에서 독립적으로 실행됨. **서킷 브레이커**가 반복적 도구 사용 패턴을 감지하여 무한 루프를 방지함.
...
완료/실패/강제 중단 모두 `backgroundNotificationHook`을 통해 메인 세션에 알려짐.

1. **이력 저장** — `background tasks json`에 기록
2. **메인 세션 알림** — `chat.message` 응답 사이클에 결과 주입
3. **컨텍스트 업데이트** — Sisyphus가 결과를 받아 후속 작업 재개

이 구조로 메인 에이전트는 백그라운드 태스크 완료를 기다리지 않고 다른 작업을 수행할 수 있음.

---

## 10. Ralph Loop (자기 참조 개발 루프)

`/ralph-loop` 또는 `/ulw-loop` 명령으로 시작되는 자기 참조 개발 루프임.
...
도구 실행은 LLM이 텍스트 응답을 반환할 때까지 반복됨.
...
사용자는 `/stop-continuation`으로 언제든 연속 실행을 중단할 수 있음.
