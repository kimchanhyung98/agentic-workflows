...
[워크플로우 다이어그램](/.draft/dev-automation/00-workflow.md)의 7계층 아키텍처를 구현 수준으로 풀어낸 문서임.
각 레이어별로 **무엇을 구현해야 하는지**, **어떤 기술 선택지가 있는지**, **구현 시 주의할 점**을 다룸.
...
외부 채널에서 들어오는 다양한 형태의 요청을 단일 `task request` 포맷으로 변환함.
이 레이어 자체는 AI가 필요 없는 결정론적 변환 계층임.
...
| **메신저 (Slack, Discord)** | 웹훅 → 브리지 서버 → AI CLI 프로세스 | 메시지 파싱, 멘션 추출, thread context를 포함 |

### task request 스키마

모든 채널의 요청은 아래 포맷으로 정규화되어야 함.
...
**채널 추가의 확장성**: Intake 레이어는 어댑터 패턴으로 설계함.
새 채널 추가 시 정규화 어댑터만 작성하면 됨. 하류 레이어는 `task_request` 스키마만 인식함.

**입력 검증**: 정규화 직후 필수 필드 검증을 수행함.
`repo`가 유효한 Git 저장소인지, `prompt`가 비어 있지 않은지 등의 기본 검증임.
검증 실패 시 즉시 reject하고 요청자에게 오류를 반환함.

**인증과 인가**: 메신저 채널의 경우 웹훅 서명 검증이 필요함.
CLI 직접 실행은 로컬 사용자이므로 별도 인증이 불필요함.
...
`task_request`를 받아 `run`을 생성하고, 실행 상태를 추적하며, 동시 실행을 조율함.
같은 저장소에 대한 중복 실행을 방지하고, busy 상태일 때 요청을 큐에 적재함.
...
State Store는 run의 전체 생명주기를 추적함.
...
초기 구현은 **SQLite**를 권장함.
파일 기반이라 배포가 간단하고, 트랜잭션으로 동시성 문제를 해결하며, run 이력 조회에 SQL을 쓸 수 있음.

### 동시 실행 정책

같은 repo에 대해 동시에 여러 run이 실행되면 workspace 충돌이 발생함.
...
큐는 State Store와 동일한 저장소에 구현함.
복잡한 메시지 브로커(RabbitMQ, Kafka)는 초기에 불필요함.
SQLite 테이블 하나(`pending_tasks`)로 FIFO 큐를 구현하고, polling으로 dequeue함.
...
이 레이어는 시스템에서 가장 중요한 레이어임.
**기획의 품질이 구현의 품질을 결정함** — Plan Doc이 부실하면 이후 모든 레이어가 무의미해짐.

이 레이어의 AI CLI는 **코드 변경 권한이 없음**. 읽기와 문서 생성만 허용됨.
...
Plan Doc은 마크다운 파일로 저장하되, 프론트매터에 메타데이터를 포함함.
...
Main AI CLI는 일반적인 AI CLI 세션이되, 시스템 프롬프트에 제약을 명시함.
...
멀티 AI 리뷰를 통과한 Plan Doc을 사람에게 제시하고, 승인/반려를 받음.
승인 시 Policy Gate를 거쳐 Execution Grant를 발급함.

### Human Approval 구현

사람에게 Plan Doc을 제시하는 방식은 진입 채널에 따라 다름.
...
**승인 대기 타임아웃**: 설정 가능한 타임아웃(기본: 24시간)을 두고, 초과 시 run을 `escalated`로 전이함.

### Policy Gate 구현

Policy Gate는 사람의 승인과 별개로, 시스템이 강제하는 규칙임.
코드로 구현되며, AI가 우회할 수 없음.
...
`--auto` 모드는 사람 승인을 생략하지만, 아래 조건을 모두 충족해야 함.
...
하나라도 위반 시 `human-reviewed` 모드로 fallback함.
...
Execution Grant를 받아 격리된 workspace를 준비하고, Executor AI CLI가 Plan Doc을 기준으로 코드를 수정함.
이 레이어의 AI CLI는 **승인된 workspace 내부에서만 쓰기 권한**을 가짐.
...
Executor에게 주입하는 환경변수는 최소 범위로 제한함.
...
**구현**: `.agent/secrets/{run_id}.env` 파일을 Execution Grant 발급 시 생성하고, run 종료 시 삭제함.

### Executor AI CLI 실행

Executor는 Plan Doc을 입력으로 받아 코드를 수정함.
...
**병렬 실행**: Plan Doc의 작업 계획이 독립적인 파일 그룹으로 나뉠 경우, 복수 Executor를 병렬 실행할 수 있음.
단, 파일 간 의존성이 있으면 직렬 실행해야 함.
...
Executor의 결과물을 **내부 루프**(결정론적 Gate)와 **외부 루프**(멀티 AI 리뷰)로 검증함.
실패 시 구조화된 피드백을 생성하여 Executor에게 수정을 지시함.

### 내부 루프: Deterministic Gates

Gate는 하드코딩된 스크립트로, AI가 우회할 수 없음.
...
**핵심 원칙**: Gate는 프로젝트의 기존 도구를 그대로 사용함.
`make check`, `npm run lint`, `pytest` 등 프로젝트에 이미 있는 명령어를 호출함.
에이전트 전용 검증 도구를 만들지 않음.

**Gate 실패 시**: 오류 메시지를 Executor에게 전달하고 즉시 수정을 시도함.
이 내부 루프는 외부 retry 카운트에 포함하지 않음.
단, 내부 루프도 최대 횟수를 설정함(기본: 5회). 초과 시 외부 retry로 전이함.

### Selective / Integration Checks

Gate를 모두 통과한 후, 선택적으로 통합 테스트를 실행함.

**실행 조건**: 프로젝트에 통합 테스트가 존재하고, Plan Doc의 수정 범위가 통합 테스트 대상에 해당할 때만 실행함.
모든 run에 통합 테스트를 돌리면 비용과 시간이 과도함.
...
Gate를 통과한 diff를 복수 AI 모델이 병렬로 검증함.

#### 리뷰어 실행 구현

각 리뷰어는 독립된 AI CLI 프로세스로 실행됨.
...
Judge는 복수 리뷰 결과를 종합하여 최종 판정을 내림.
Judge 자체는 결정론적 규칙으로 구현하거나, AI를 사용할 수 있음.
...
Judge가 fail을 판정하면, 모든 리뷰어의 findings를 수정 가능한 피드백으로 변환함.
...
검증을 통과한 결과물을 commit/PR로 발행하거나, 실패 리포트를 작성함.
**publish 권한은 이 레이어만 보유함.**
...
retry 초과 또는 escalate 판정 시 실패 리포트를 작성함.
...
모든 run의 산출물을 보관함. 디버깅, 감사, 학습에 활용함.
...
산출물 완성 후 요청자에게 알림을 보냄. 진입 채널에 따라 방식이 다름.
...
프로젝트별 설정은 `.agent/config.yaml`에 저장함.
...
이 단계에서는 하네스 코드가 거의 없음.
AI CLI를 수동으로 실행하되, Plan Doc 작성 → 승인 → 실행 → Gate의 흐름을 사람이 조율함.
**핵심은 흐름을 먼저 검증하는 것**임.
...
이 단계에서 사람은 Plan Doc 승인만 하고, 나머지는 자동임.
