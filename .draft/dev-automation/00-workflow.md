...
Agentic Workflow와 Design Pattern의 패턴 분류를 종합한 로컬 개발 자동화 아키텍처임.

AI CLI를 역할별 런타임으로 분해하고, 그 주변에 상태, 권한, 게이트, 재시도, 승인, 발행을 구조로 강제해야 운영 가능한 개발 자동화 시스템이 됨.

다만 모든 작업에 동일한 무게의 워크플로우를 강제하면 로컬 자동화의 장점이 사라짐.
이 문서의 권장 기본값은 `위험도 기반 라우팅 + Plan Doc으로 범위 고정 + single-writer execution + review depth 차등화`임.
...
> 로컬 에이전트: 에이전트를 만드는 것이 아니라, **에이전트가 동작하는 환경을 설계**함.

---

## 1. 전체 시스템 구조

이 시스템은 단일 AI가 처음부터 끝까지 직접 처리하는 구조가 아님.
`입력 수집 → 실행 문맥 생성/위험도 분류 → 컨텍스트/기획 → 승인/정책 → 격리 실행 → 검증/복구 → 산출물 발행`의 7개 레이어를 따라, 여러 AI CLI 세션이 서로 다른 권한으로 협력함.
...
각 레이어는 무엇을 받아서 무엇을 내보내는지, 어디까지 권한을 가지는지, 실패하면 어디로 되돌아가는지가 명시되어야 함.
...
7개 레이어는 공통 골격이고, 실제 운영에서는 `change class`에 따라 깊이를 줄이거나 늘려야 함.
모든 run에 고정된 3중 리뷰와 full isolation을 강제하면 로컬 자동화의 처리량이 급격히 떨어짐.
...
- 등급 판단 기준은 수정 파일 수보다 `민감한 경로`, `외부 side effect`, `롤백 난이도`, `미해결 판단의 수`를 우선함.
- 등급 자체는 Plan Doc의 필드를 늘리기보다 `run context`와 `execution grant` 메타데이터로 관리하는 편이 나음.

---

## 3. AI CLI 역할 분리

AI CLI는 단일 주체가 아니라 역할별 런타임임.
같은 종류의 CLI를 써도 세션과 권한을 분리해야 하며, `메인 오케스트레이터`, `실행자`, `리뷰어`, `판정자`가 같은 디렉토리와 같은 권한을 공유하면 구조가 무너짐.
특히 실행자는 기본 single-writer로 두고, 병렬화는 파일 소유권이 겹치지 않고 별도 workspace를 발급할 수 있을 때만 허용해야 함.
...
정적 구조와 별개로, 실제 런타임에서는 아래 순서로 상호작용이 진행됨.
핵심은 `Main AI CLI`가 전체 흐름을 조율하되, `plan review`, `human approval`, `workspace execution`, `deterministic gate`, `publish`가
명확히 분리된다는 점임. 또한 `L2`가 먼저 작업 등급과 review depth를 계산하고, `--auto` 조건을 만족하지 못하면 사람 승인 경로로 자동 강등됨.
...
준비 단계는 `Prompt Chaining`에 가까운 결정론적 파이프라인임.
핵심은 **코드 쓰기 전에 Plan Doc으로 범위를 고정**하고, L2가 계산한 change class에 따라 review depth와 approval 경로가 정해진 뒤에만 write 권한이 열리는 것임.
...
Augment Context MCP를 활용하여 요청을 분석하고 프로젝트 내부 정보를 파악함.
...
프로젝트 외부의 정보가 필요한 경우에만 수행함.
AI가 요구사항 분석 결과를 바탕으로 추가 조사 필요 여부를 판단함.
...
프로젝트에 사전 구조화된 AI 컨텍스트 파일이 존재하는 경우 로딩함.
도메인 개요, 데이터 모델, API 스펙 등 아키텍처 레이어에 매핑된 정적 지식임.
AI 컨텍스트가 없는 프로젝트에서는 이 단계를 건너뜀.

### Plan Doc 필수 필드

Plan Doc은 시스템의 핵심 산출물이자 계층 간 계약(contract)임.
...
Plan Doc은 길게 쓰는 문서가 아니라 `scope freeze` 문서임.
이 6개 필드는 execution grant, gate 범위, review 기준의 원본이 되어야 함.
...
실행 plane의 핵심 원칙은 `workspace boundary = permission boundary`임.
`로컬 / VM / Docker` 선택은 단순한 환경 옵션이 아니라, **실행 권한과 실패 영향 범위를 결정하는 아키텍처 요소**로 취급해야 함.
권장 기본값은 `single-writer + short-lived workspace`임. 병렬 executor는 파일 집합이 겹치지 않고 shard별 workspace와 merge 계약이 있을 때만 여는 것이
안전함.
...
1. **승인 전 write 금지**: plan 단계 AI CLI는 코드 수정 권한이 없음.
2. **workspace 밖 쓰기 금지**: executor는 main repo가 아니라 worktree 또는 sandbox 안에서만 수정함.
3. **publish 권한 분리**: commit / PR 생성은 output layer만 수행함.
4. **secret scope 축소**: execution에 꼭 필요한 환경변수만 주입함.
5. **네트워크 정책 외부화**: executor 프롬프트가 아니라 infra 정책으로 제어함.

### 실행 모드 선택

실행 모드는 단순 UX 옵션이 아니라, 승인 경로와 권한 범위를 바꾸는 정책 스위치임.
기본값은 `human-reviewed`이고, `--auto`는 격리 조건이 충분할 때만 열리는 고위험 모드로 취급함.
...
검증 plane은 `Stripe Minions`의 결정론적 게이트와 `Evaluator-Optimizer` 패턴을 조합한 구조임.
핵심은 **빠른 내부 루프**(결정론적 Gate)와 **비싼 외부 루프**(멀티 AI 리뷰)를 분리하는 것임.
외부 루프의 깊이는 고정 3중 리뷰가 아니라 작업 등급에 따라 `1 → 2 → 3`으로 늘리는 편이 현실적임.
...
아래 분업은 `high-risk` 기준임. `small`은 A만, `standard`는 A+B까지로 축소하는 것을 기본값으로 둠.
...
`plan review`와 `code review`는 서로 다른 시스템이 아니라, 같은 review harness를 다른 입력으로 재사용하는 구조임.
이 모듈은 병렬 리뷰어, 결과 집계기, 판정 규칙으로 구성되며 준비 단계와 실행 단계 모두에서 호출됨.
리뷰어 수는 아키텍처의 상수가 아니라 `change class`의 함수여야 함.
...
운영 가능한 시스템은 상태 전이를 명시해야 함.
아래 상태도는 `요청 수신`에서 `PR 발행` 또는 `실패 보고`까지의 핵심 경로와, 각 실패가 어디로 되돌아가는지를 보여줌.
...
이 시스템은 코드만 만드는 것이 아니라, 단계별 산출물을 생산하고 다음 레이어가 그 산출물을 입력으로 소비하는 구조임.
`입력/출력`이 불분명하면 아키텍처가 아니라 프롬프트 모음이 됨.
...
여기서부터는 본문 아키텍처의 설계 근거를 비교 관점에서 정리함.
핵심 본문은 1-10절이고, 아래 내용은 외부 시스템과의 차이를 통해 왜 이런 구조를 선택했는지 설명하는 보조 섹션임.

### 안전성 모델 비교

이 최종 구조는 `격리 기반`, `등급 기반`, `샌드박스 기반`, `리뷰 기반` 모델의 중간 지점에 위치함.
기본값은 `리뷰 = 권한`이지만, 실행 모드와 실행 경계에 따라 `격리 = 권한` 방향으로 이동할 수 있게 설계함.
...
현재 아키텍처는 단일 원본을 베낀 것이 아니라, 각 시스템에서 유효했던 구조 요소를 선택적으로 결합한 것임.
...
이 아키텍처는 하나의 패턴으로 설명되지 않음.
레이어별로 서로 다른 패턴을 조합해야 전체 시스템이 안정적으로 동작함.
...
이 문서의 최종 구조를 한 줄로 줄이면, `AI CLI를 엔진으로 쓰고 하네스를 시스템으로 설계한다`임.
