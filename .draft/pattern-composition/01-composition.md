...
개별 패턴을 결합하여 더 큰 시스템을 구성한 4가지 사례임.
각 사례는 단일 프로덕션 시스템에서 일반화한 것이며, base 패턴은 이 레포의 [설계 패턴](/design-pattern/README.md) 문서를 참조함.

---

## 사례 1: 게이트 워크플로우

4가지 base 패턴을 결합하여 Phase 간 Gate가 있는 워크플로우를 구성함.
...
- **순차 패턴**은 실패 시 재시도를 정의하지 않음. Phase가 실패하면 파이프라인이 멈춤.
- **검토-비평 패턴**은 생성-비평 사이클을 설명하지만, 비평 실패 시 "어디로 돌아갈지"를 정의하지 않음.
- **인간 참여형 패턴**은 체크포인트와 에스컬레이션을 다루지만, 이 레포의 base 문서는 자동 판정과 인간 판정의 구체적 전환 규칙(severity 임계값, 변경 등급별 정책)까지는 명시하지 않음.
...
이 중 1, 2, 3은 base 패턴 문서에 정의되지 않은 계약임.

---

## 사례 2: 분석 파이프라인

3가지 base 패턴을 결합하여 코드 분석 파이프라인을 구성함.
...
- **순차 패턴**은 중간 단계 실패 시 처음부터 재실행해야 함. 대규모 분석에서는 비용이 급증함.
- **병렬 패턴**은 병렬 결과를 병합하는 방법을 정의하지 않음. 두 Writer가 모순되는 결과를 내면 처리 방법은?
- **계층적 분해 패턴**은 분해 구조를 설명하지만, 하위 단계 결과를 상위로 합성하는 규칙을 정의하지 않음.
...
3가지 base 패턴을 결합하여 다중 에이전트 판정 시스템을 구성함.
...
- **병렬 패턴**은 독립 실행을 다루지만, 결과가 모순될 때의 판정 규칙을 정의하지 않음.
- **코디네이터 패턴**은 동적 라우팅을 설명하지만, 라우팅된 모드가 실패했을 때의 강등/승격 경로를 정의하지 않음.
- **맞춤 로직 패턴**은 코드 기반 분기를 허용하지만, 어떤 규칙 순서가 결정론적인지 제시하지 않음.
...
여러 에이전트가 같은 Phase 내에서 협업해야 할 때 사용한 조합임. 직접 통신 대신 공유 작업 공간(Blackboard)을 통해 간접 통신하며, 결정론적 제어와 AI 판단을 3개 레이어로 분리함.
...
- **병렬 패턴**은 독립 실행을 다루지만, 워커끼리 어떻게 중간 결과를 공유하는지 정의하지 않음. 메시지를 직접 주고받으면 워커가 다른 워커의 내부 포맷에 결합됨.
- **계층적 분해 패턴**은 분해 구조만 설명함. "무엇을 코드로 결정하고 무엇을 LLM이 판단하는가"의 경계는 정의하지 않음.
- **검토-비평 패턴**은 생성-비평 사이클을 설명하지만, 워커가 상대 워커의 artifact를 수정하는 것을 막지 않음. Reviewer가 Writer의 산출물을 직접 고치면 추적성이 사라짐.

### 조합으로 필요해진 것

1. **3-레이어 책임 분리** — 턴 순서·종료 조건·상태 저장·타임아웃은 결정론적 코드에, 업무 분석·워커 배정·Stop/Go는 Leader AI에, 탐색·생성·검증은 Worker AI에 배정. 경계 규칙: Gate 평가를 Leader에 맡기지 않고, 상태 전이를 Worker에 맡기지 않음.
2. **Blackboard 간접 통신** — 워커는 공유 파일 시스템(`board/` 산출물 디렉토리, `discussion/` 턴별 대화 디렉토리)을 통해서만 통신함. 상대 워커의 포맷이나 실행 상태에 의존하지 않음.
3. **Write Scope 격리** — 각 워커는 정해진 경로에만 쓸 수 있음. Writer는 산출물을, Reviewer는 토론 파일만. 이로 인해 Reviewer가 산출물을 직접 고쳐 추적성이 사라지는 문제를 원천 차단함.
4. **턴 예산** — 전체 루프에 최대 턴 수(예: 3회) 상한. 종료 조건은 결정론적으로 파싱(`findings` 배열에 CRITICAL 0건 등)하여 Leader의 판단 오류가 무한 루프를 만들지 않게 함.

base 패턴 문서는 워커 간 통신 매체, 쓰기 범위 격리, 턴 예산 같은 운영 규칙을 다루지 않으므로, 1~4는 조합으로 필요해진 규약임.
