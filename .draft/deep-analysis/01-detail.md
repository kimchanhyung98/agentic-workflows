...
대규모 프로젝트의 전체 소스코드를 AI로 분석하는 워크플로우임.
소스코드를 **XML Bundling**으로 구조화하고, **3단계 점진적 리뷰**로 파일 수준부터 프로젝트 수준까지 검증함.
...
| 레이어             | 역할                              | 상세 섹션                                 |
|-----------------|---------------------------------|---------------------------------------|
| Layer 1: Input  | 프로젝트 소스코드, 리뷰 설정                | [Input](#layer-1-input)               |
| Layer 2: Bundle | 파일 수집 · 필터링, 단계별 XML Bundling   | [XML Bundling](#xml-bundling-layer-2) |
| Layer 3: Review | 3단계 점진적 리뷰 — 파일 → 기능 도메인 → 프로젝트 | [3단계 점진적 리뷰](#3단계-점진적-리뷰-layer-3)     |
| Layer 4: Output | 단계별 리뷰 문서 (3개) + 최종 리뷰 리포트      | [Output](#layer-4-output)             |

### Layer 1: Input

분석 대상 프로젝트와 리뷰 설정을 입력함.

**프로젝트 소스코드** — 분석 대상이 되는 프로젝트 디렉토리 전체. Git 저장소를 기준으로, 추적 대상 파일이 수집 범위가 됨.

**리뷰 설정** — 분석의 범위와 관점을 정의함.
...
3단계 리뷰에서 생성된 리뷰 문서를 최종 리포트로 종합함.
...
범위를 점진적으로 넓히며, 이전 단계에서 감지하지 못한 이슈를 보완함.
각 단계마다 독립적인 **리뷰 문서**를 산출함.
...
하나의 기능을 구성하는 파일 묶음의 일관성과 상호작용을 검증함.
...
## XML Bundling (Layer 2)

소스코드를 LLM이 이해할 수 있는 구조화된 형식으로 변환함.
리뷰 단계에 따라 번들 범위가 달라짐.
...
`<content>` 내부의 원문 코드는 반드시 XML-escaped 텍스트로 저장함.
`<`, `>`, `&`, `"`, `'` 같은 문자는 엔티티로 변환하여, HTML/XML/JSX나 문자열 리터럴이 포함되어도 문서 전체가 항상 유효한 XML이 되도록 함.

> 아래 XML 예시에서는 가독성을 위해 이스케이프를 생략하고 원문 그대로 표기함. 실제 번들에서는 XML 엔티티로 변환해야 함.

#### 태그 설명

| 태그                   | 속성                                  | 설명                                                                                                          |
|----------------------|-------------------------------------|-------------------------------------------------------------------------------------------------------------|
| `<review>`           | `target`, `domain`, `scope`, `name` | 리뷰 단위를 정의함. `target`(파일 경로), `domain`(기능 도메인), `scope='project'`(프로젝트)로 범위를 지정하고, `name`으로 프로젝트 식별자를 지정함. |
...
각 소스 파일마다 개별 XML을 생성함.
대상 파일(`role="target"`)과 해당 파일이 import/참조하는 관련 파일(`role="context"`)을 함께 포함함.
...
하나의 기능을 구성하는 관련 파일을 모두 묶어 하나의 XML을 생성함.
해당 기능의 모든 파일이 리뷰 대상(`role="target"`)임.
...
- 같은 기능을 구성하는 view, model, serializer, test 등을 하나로 묶음
- 디렉토리 구조가 아닌 **기능/도메인** 단위로 그룹핑함
- 예: `auth` 도메인 = `views/auth.py` + `models/user.py` + `serializers/user.py` + `tests/test_auth.py`

### 3단계 번들: 프로젝트 단위

원본 코드 대신, 이전 단계의 리뷰 결과와 프로젝트 메타데이터를 입력으로 구성함.
...
일반 텍스트 방식(`=== file.py ===`)은 파일 경계 구분이 모호하고 메타데이터를 표현하기 어려움.
XML은 태그로 경계를 명확히 하고, 속성으로 메타데이터를 자연스럽게 포함함.

---

## 멀티 AI 리뷰

각 단계에서 복수의 AI 모델이 서로 다른 관점으로 병렬 검증함.
[개발 워크플로우의 멀티 AI 리뷰](/.draft/dev-automation/01-detail.md#멀티-ai-리뷰-구조)와 동일한 구조를 적용함.
...
리뷰어 구성은 프로젝트 특성에 따라 조정함.
예: 프론트엔드 프로젝트는 접근성·UX 관점을 추가하고, 데이터 파이프라인은 데이터 정합성 관점을 추가함.
...
- 개발 워크플로우는 **코드 변경(diff) 기준**으로 리뷰하지만, Deep Analysis는 **프로젝트 전체**를 분석함
- 개발 워크플로우는 기획 문서 대조가 핵심이지만, Deep Analysis는 **일반적인 코드 품질 관점**으로 분석함
- Deep Analysis는 **3단계 점진적 리뷰**로 파일 수준부터 프로젝트 수준까지 범위를 확장함
- Deep Analysis에는 자동 수정 루프가 없음 — 리포트를 산출물로 제공함
