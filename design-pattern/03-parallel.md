...
병렬 패턴은 여러 전문화된 하위 에이전트가 동시에 독립적으로 작업을 수행하고, 그 결과를 종합하여 최종 출력을 생성하는 멀티 에이전트 패턴임.
...
flowchart TD
    ...
    subgraph Workflow["병렬 워크플로"]
        ...
        Dispatcher --> A1["에이전트 1<br/>감정 분석"]
        Dispatcher --> A2["에이전트 2<br/>키워드 추출"]
        Dispatcher --> A3["에이전트 3<br/>카테고리 분류"]
        Dispatcher --> A4["에이전트 4<br/>긴급도 감지"]
