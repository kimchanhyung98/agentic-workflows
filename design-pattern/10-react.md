...
ReAct 패턴은 AI 모델을 사용하여 사고 과정(Reasoning)과 행동(Acting)을 자연어 상호작용의 시퀀스로 구성하는 패턴입니다. 에이전트는 추론(Think) → 행동(Act) → 관찰(Observe)의
반복 루프를 통해 작업을 수행합니다.
...
flowchart TD
    ...
    subgraph ReActLoop["ReAct 루프"]
        Agent[에이전트] --> Think["추론 Think<br/>현재 상태 분석<br/>다음 행동 계획"]

        Think --> Complete{"작업<br/>완료?"}

        Complete -- "아니오" --> Act["행동 Act<br/>도구 선택 및 실행"]
        Complete -- "예" --> Answer[최종 답변 구성]

        Act --> Tools
        Tools --> Observe["관찰 Observe<br/>도구 출력 수신<br/>메모리 저장"]
        Observe --> Think
    end
...
flowchart LR
    subgraph Iteration1["반복 1"]
        T1["Think<br/>목표 이해<br/>첫 번째 행동 계획"]
        A1["Act<br/>웹 검색 실행"]
        O1["Observe<br/>검색 결과 분석"]
    end

    subgraph Iteration2["반복 2"]
        T2["Think<br/>추가 정보 필요<br/>다음 행동 계획"]
        A2["Act<br/>API 호출"]
        O2["Observe<br/>API 결과 분석"]
    end

    subgraph Iteration3["반복 3"]
        T3["Think<br/>충분한 정보 확보<br/>최종 답변 구성"]
        A3["Act<br/>답변 생성"]
    end
...
| 구분    | 내용                                     |
|-------|----------------------------------------|
| ✅ 장점  | 복잡한 멀티 에이전트 시스템보다 구현, 유지보수, 비용 면에서 효율적 |
