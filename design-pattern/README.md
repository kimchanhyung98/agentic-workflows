...
문서를 기반으로 정리한 에이전틱 AI 시스템의 설계 패턴임.
...
> **참고**: 워크로드가 예측 가능하고, 구조화되어 있으며, 단일 모델 호출로 실행 가능하다면 에이전트가 아닌 솔루션(요약, 번역, 분류 등)을 먼저 고려할 것.
...
> **참고**: 반복 개선 패턴은 결정론적 워크플로와 반복 포함 워크플로 양쪽에서 활용 가능함. 사전 정의된 개선 루프로 사용하면 결정론적, 품질 기준에 따른 동적 반복으로 사용하면 반복 포함 워크플로에
> 해당함.
...
> 아래 표는 Google Cloud 원문의 각 패턴별 설명을 바탕으로 재구성한 요약임.
