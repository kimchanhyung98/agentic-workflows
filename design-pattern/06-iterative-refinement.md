...
반복 개선 패턴은 하나 이상의 에이전트가 루프 내에서 세션 상태에 저장된 결과물을 여러 사이클에 걸쳐 점진적으로 개선하는 멀티 에이전트 패턴임.
...
flowchart TD
    ...
    subgraph RefinementLoop["반복 개선 루프"]
        Init[초기 작업] --> Agent["에이전트<br/>결과물 생성/수정"]
        Agent --> State[("세션 상태<br/>결과물 저장")]
        State --> Eval{품질 평가}

        Eval -- "❌ 기준 미충족<br/>(반복 계속)" --> Agent
        Eval -- "✅ 기준 충족<br/>또는 최대 반복 도달" --> Done[루프 종료]
    end
...
> **참고**: 두 패턴 모두 루프 패턴의 구현이며 세션 상태를 활용할 수 있으나, 검토-비평은 승인/거부 판정에, 반복 개선은 결과물의 버전 관리에 초점을 둠.
