...
이 문서에서는 정보의 근거를 세 단계로 구분한다.

| 라벨        | 의미                                                  |
|-----------|-----------------------------------------------------|
| **공식**    | Coinbase 공식 블로그에서 직접 확인                             |
| **공개 발언** | Chintan Turakhia의 인터뷰, LinkedIn/X 공개 포스트, 팟캐스트 쇼노트  |
| **2차 정리** | LangChain 블로그, Ry Walker Research, mager.co 등 외부 분석 |

### 공식적으로 확인된 기반 인프라

| 항목         | 공개 내용                                                                                         | 근거                                                             |
|------------|-----------------------------------------------------------------------------------------------|----------------------------------------------------------------|
| 개발자용 AI 도구 | Cursor, Copilot, Claude Code 등을 전사적으로 도입                                                      | `Tools for Developer Productivity`, 2025-08-06 `[공식]`          |
| 내부 MCP 통합  | GitHub, Linear 중심의 MCP 통합                                                                     | `Tools for Developer Productivity`, 2025-08-06 `[공식]`          |
| 모델 인프라     | OpenAI-compatible router, 일일 1,500명+ 사용                                                       | `Tools for Developer Productivity`, 2025-08-06 `[공식]`          |
| 접근 제어      | repository sensitivity matrix로 민감 리포지토리 접근 제어                                                 | `Tools for Developer Productivity`, 2025-08-06 `[공식]`          |
| 에이전트 운영 원칙 | code-first, observability-first, tracing, evaluation harness, auditability, human-in-the-loop | `Building enterprise AI agents at Coinbase`, 2025-12-22 `[공식]` |
...
다른 회사들이 오픈소스 에이전트를 포크하거나 기존 프레임워크 위에 구성한 것과 달리, Coinbase는 Cloudbot을 처음부터 직접 만든 사례로 반복 언급된다. `[2차 정리 + 공개 발언]`
...
Coinbase 공식 블로그에서도 **repository sensitivity matrix**를 운영하여 민감 리포지토리에 대한 에이전트 접근을 제어한다고 밝혔다. `[공식]`
...
참고: Coinbase 공식 블로그에서는 **OpenAI-compatible router**를 운영하며 1,500명 이상의 엔지니어가 매일 사용한다고 밝혔다. `[공식]`

### 3.2 Linear-first 컨텍스트 `[공개 발언]`
...
공식 블로그에서 에이전트를 **코드-우선, 관측 가능, 평가 가능, 감사 가능**하게 구축한다고 밝혔다. `[공식]`

### 3.4 MCPs + Custom Skills `[2차 정리]`
...
## 4. 공개 자료에 반복 등장하는 3가지 상호작용 패턴 `[공개 발언 / 2차 정리]`
...
## 5. 검증 및 승인 파이프라인 `[공식 + 2차 정리]`

Coinbase 공식 블로그에서 직접 확인되는 검증 요소는 아래와 같다.

- 모든 도구 호출, 검색, 결정, 출력이 **추적(traced)**됨 `[공식]`
- 결정론적 데이터 단계와 LLM 단계를 분리하고, LLM 단계는 **evaluation harness**와 큐레이션된 데이터셋으로 관리 `[공식]`
- **human-in-the-loop**를 의도적인 시스템 구성 요소로 둠 `[공식]`
- 각 실행마다 입력, 사용 데이터, 판단 과정, 승인 주체를 남기는 **immutable record**를 생성 `[공식]`
...
| 지표             | Before | After         | 출처                 |
|----------------|--------|---------------|--------------------|
| PR 리뷰 시간       | ~150시간 | ~15시간         | 공개 발언, 2026-03-02  |
| 피드백 → 기능 배포    | 수 주    | 수 분           | 공개 발언, 2026-03-02  |
| AI 에이전트 PR 비율  | —      | 전체 머지 PR의 ~5% | 공개 발언 미러 / 2차 정리   |
| 엔지니어 AI 플랫폼 사용 | —      | 1,500명+ 매일 사용 | 공식 블로그, 2025-08-06 |

---

## 8. 3사 내부 코딩 에이전트 비교 `[2차 정리]`
...
| 요소    | 접근 방식                                       | 출처    |
|-------|---------------------------------------------|-------|
| 아키텍처  | 완전 자체 구축 (보안·컴플라이언스 우선)                     | 2차 정리 |
| 컨텍스트  | Linear 중심 수집                                | 공개 발언 |
| 검증    | Agent Council + 위험도별 자동/수동 머지               | 2차 정리 |
| 채택    | PR 스프린트, Slack 채널 기반 자발적 확산                 | 공개 발언 |
| 운영 원칙 | Observability-first (tracing, eval harness) | 공식    |

### code-first graph 패턴 `[공식]`
