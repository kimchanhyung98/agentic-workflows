...
- 주요 목표: Slack/Linear/GitHub에서 `@openswe` 호출 → 격리된 샌드박스에서 코드 수정 → Draft PR 자동 생성
...
중요한 점은 `webapp.py`가 저장소를 직접 수정하거나 `get_agent()`를 직접 호출하지 않는다는 것이다. 실제 저장소 조작은 LangGraph Runtime이 `server.py:get_agent`를 호출한 뒤 샌드박스 안에서만 일어난다.
...
| `open_pr_if_needed`                | `@after_agent`    | 변경사항 있지만 PR 미생성 시 자동 커밋·푸시·PR 생성     |
...
7. **결과 전파**: 변경사항이 있으면 PR을 만들고, 결과를 원본 채널에 회신
...
- **보안**: `GITHUB_USER_EMAIL_MAP`에 없는 사용자 코멘트는 `<dangerous-external-untrusted-users-comment>` 태그로 래핑하여 프롬프트 인젝션 방지
...
**캐시 우선 전략**: 먼저 thread metadata에서 암호화된 토큰을 찾고, 있으면 Fernet 복호화하여 재사용한다.

**source별 분기:**

- **Linear/Slack**: 이메일 → LangSmith 사용자 ID 조회 → GitHub OAuth 토큰 획득
- **GitHub (bot 모드)**: GitHub App JWT → Installation Token 직접 생성
- **GitHub (OAuth 모드)**: 로그인 → `GITHUB_USER_EMAIL_MAP` 이메일 매핑 → LangSmith OAuth 경로

인증 실패 시 `leave_failure_comment()`가 원래 트리거 소스에 실패 메시지를 게시한다.
...
- 미들웨어 4종 (큐 주입, 오류 처리, 빈 응답 방지, PR 안전망)
...
5. **결정적 안전장치**: 미들웨어가 LLM의 비결정적 행동에 결정적 제어를 추가 (PR 안전망, 오류 변환)
