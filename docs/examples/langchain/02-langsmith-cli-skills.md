...
LangSmith CLI는 `langsmith-cli` 명령으로 실행하며, 주요 기능은 다음과 같다:

| 명령어 예시                                                                              | 설명            |
|-------------------------------------------------------------------------------------|---------------|
| `langsmith-cli runs list --project my-app --status error --last 24h`                | 에러 발생 실행 조회   |
| `langsmith-cli --json runs get-latest --project X --fields inputs,outputs`          | 최근 실행 JSON 조회 |
| `langsmith-cli --json runs analyze --group-by tag:model --metrics count,error_rate` | 실행 분석         |
| `langsmith-cli runs watch --project production`                                     | 실시간 실행 모니터링   |
| `langsmith-cli runs sample --stratify-by tag:length`                                | 실행 샘플링        |
| `langsmith-cli --json runs fields --include inputs,outputs`                         | 필드 탐색         |
...
sequenceDiagram
    ...
    D->>CLI: langsmith-cli runs list
    CLI->>LS: API 호출
    LS-->>CLI: 실행 데이터
    CLI-->>D: 결과 출력

    D->>CLI: langsmith-cli runs analyze
    CLI->>LS: 분석 요청
    LS-->>CLI: 분석 결과
    CLI-->>D: 지표 출력
```
...
```bash
# 에러 발생 실행 조회
langsmith-cli runs list --project my-agent --status error --last 24h

# 특정 실행의 상세 정보 확인
langsmith-cli --json runs get-latest --project my-agent --fields inputs,outputs,error
```
...
```bash
# 실행 샘플링
langsmith-cli runs sample --project my-agent --stratify-by tag:content_type
```
...
```bash
# 평가 결과 분석
langsmith-cli --json runs analyze --group-by tag:model --metrics count,error_rate,p50_latency,avg_cost
```
