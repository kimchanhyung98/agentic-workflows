...
> 아래 XML 예시에서는 가독성을 위해 이스케이프를 생략하고 원문 그대로 표기한다. 실제 번들에서는 XML 엔티티로 변환해야 한다.

#### 태그 설명

| 태그                   | 속성                                  | 설명                                                                                                          |
|----------------------|-------------------------------------|-------------------------------------------------------------------------------------------------------------|
| `<review>`           | `target`, `domain`, `scope`, `name` | 리뷰 단위를 정의한다. `target`(파일 경로), `domain`(기능 도메인), `scope='project'`(프로젝트)로 범위를 지정하고, `name`으로 프로젝트 식별자를 지정한다. |
