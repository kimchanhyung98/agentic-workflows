...
검토-비평 패턴은 생성기(Generator) 에이전트가 초기 출력을 생성하고, 비평가(Critic) 에이전트가 사전 정의된 기준으로 해당 출력을 평가한 뒤, 기준을 충족하지 못하면 생성기에게 피드백을 보내 재생성하는 루프 패턴의 구체적 구현입니다.
