...
병렬화는 작업을 독립적인 하위 작업으로 분해하여 여러 에이전트가 동시에 처리하거나, 동일한 작업을 여러 에이전트가 독립적으로 수행하고 결과를 집계하는 설계 패턴임.
...
```mermaid
gantt
    title 순차 처리 vs 병렬 처리 시간 비교 (단위: 초)
    dateFormat X
    axisFormat %s
...
### 2. Guardrails 패턴 (섹션 처리, 원문 예시)

사용자 쿼리의 안전성 검증과 응답 생성을 병렬로 처리:

- **에이전트 A**: 사용자 쿼리를 처리하여 응답 생성
- **에이전트 B**: 동일 쿼리에 대해 부적절한 콘텐츠나 요청을 스크리닝
- **집계**: 스크리닝 통과 시에만 응답 전달, 차단 시 거부 응답 반환
- **핵심**: 동일 LLM이 가드레일과 핵심 응답을 동시에 처리하는 것보다 분리하는 것이 더 효과적
