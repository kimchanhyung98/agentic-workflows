...
| 패턴                                         | 핵심 개념                              | 적합한 상황                      |
|--------------------------------------------|------------------------------------|-----------------------------|
| [프롬프트 체이닝](/effective-agents/01-prompt-chaining.md)        | 작업을 순차적 단계로 분해, 각 LLM 출력이 다음 단계 입력 | 명확한 순서의 하위 작업, 중간 검증 필요     |
| [라우팅](/effective-agents/02-routing.md)                     | 입력 분류 후 최적화된 전문 처리기로 전달            | 입력 유형에 따라 다른 처리가 필요한 경우     |
| [병렬화](/effective-agents/03-parallelization.md)             | 독립 작업의 동시 처리 또는 다수결 투표             | 처리 속도 중요, 대용량 입력, 신뢰도 향상 필요 |
| [오케스트레이터-워커](/effective-agents/04-orchestrator-workers.md) | 오케스트레이터가 동적으로 작업 분배 및 통합           | 복잡하고 예측 불가능한 다단계 작업         |
| [평가자-최적화자](/effective-agents/05-evaluator-optimizer.md)    | 생성-평가-개선의 반복 피드백 루프                | 명확한 품질 기준으로 반복 개선이 효과적인 경우  |
| [자율 에이전트](/effective-agents/06-autonomous-agent.md)        | 도구를 자율적으로 사용하여 개방형 목표 달성           | 장기 실행, 예측 불가한 개방형 목표        |
