...
오케스트레이터-워커 패턴은 중앙의 오케스트레이터 에이전트가 작업을 동적으로 분해하고 전문화된 워커 에이전트들에게 위임하며, 워커들의 결과를 취합하여 최종 응답을 생성하는 설계 패턴임.
...
- 병렬화: 하위 작업이 사전에 정의되어 있고 고정된 방식으로 병렬 처리
- 오케스트레이터-워커: 오케스트레이터가 입력에 따라 런타임에 하위 작업을 동적으로 결정
...
> 점선 화살표는 오케스트레이터가 런타임에 동적으로 워커를 선택하고 작업을 위임함을 나타냄. 워커 수와 작업 내용은 고정되지 않음. 오케스트레이터 자체가 워커의 결과를 종합(synthesize)하여 최종
> 출력을 생성함.
