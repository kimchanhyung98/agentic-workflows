...
- Renderer는 preload API를 통해 `TASK_START`, `TASK_STOP`, `TASK_REVIEW` 같은 IPC 이벤트를 발행함.
- `ipc-setup.ts`는 실제 구현을 `ipc-handlers/*` 도메인 모듈로 위임함.
- `task/execution-handlers.ts`는 git repo 여부, 초기 커밋 존재, 인증 상태, spec/plan 파일 상태를 확인하고 실행 분기를 결정함.
...
- `taskStateManager`는 XState actor를 통해
  ...
  `executionPhase`를 `implementation_plan.json`에 동기화함.

**의도:** UI 상태, worker 이벤트, plan 파일 상태를 한 곳에서 정렬해 "카드 위치는 바뀌었는데 실제 plan은 다른 상태" 같은 불일치를 줄이는 데 초점이 있음.

### 2.2 Task Runtime 계층

- `AgentManager`는 task/spec/QA 실행의 진입점 facade임.
- `AgentProcessManager`는 worker thread spawn, 종료, 재시작, event relay를 담당함.
- `AgentEvents`는 worker의 structured progress를 우선 사용하고, 필요할 때만 로그 텍스트를 fallback으로 파싱해 phase를 추정함.
- `taskExecutionContext` Map에는 `projectPath`, `specId`, `options`, `swapCount`, `generation`, `projectId` 등이 저장되어
  restart 시 동일 컨텍스트를 재구성함.
- `AgentQueueManager`는 이름과 달리 일반 task 빌드 큐가 아니라 **roadmap / ideation 러너의 queue와 progress persistence**를 담당함.

### 2.3 AI 실행 계층

- build 실행 시 `createOrGetWorktree()`가 `.auto-claude/worktrees/tasks/{specId}` 아래 worktree를 준비함.
- `worker.ts`는 serialized session config를 받아 security profile과 tool context를 복원하고, MCP clients를 초기화한 뒤 agent type별 실행
  경로를 선택함.
- `build_orchestrator`, `qa_reviewer`, `spec_orchestrator`는 각각 `BuildOrchestrator`, `QALoop`, `SpecOrchestrator`로 라우팅됨.
- 그 외 agent type은 단일 세션 경로를 타며, `runContinuableSession()`이 내부적으로 AI session runner를 감쌈.
- `ToolRegistry`는 내장 도구 9개와 MCP 도구를 합성하고, `Tool.define`은 security hook과 write-path containment를 적용함.

### 2.4 오케스트레이션 계층

에이전트 하나가 전체 작업을 끝내는 구조가 아니라, phase별 agent와 orchestrator가 파일 산출물 중심으로 이어 붙는 구조임.

**SpecOrchestrator** — complexity-first spec 생성:

1. `complexity_assessment`를 먼저 실행함.
...
**BuildOrchestrator** — spec을 구현으로 전환:

1. `planner`가 `implementation_plan.json`을 생성하거나 정규화함.
2. `coder`가 `SubtaskIterator`를 통해 subtask를 순차/병렬로 수행함.
3. `qa_reviewer`가 결과를 검증함.
4. 실패 시 `qa_fixer`가 수정하고 다시 QA를 돎.
5. QA 통과 후 최종 human review 단계로 넘어감.

**QALoop** — review/fix 반복:

- `qa_reviewer → qa_fixer → qa_reviewer` 루프를 돌며 반복 검증함.
- 반복 실패, recurring issue, consecutive error가 일정 수준을 넘으면 escalation 쪽으로 빠짐.
- `QA_FIX_REQUEST.md`가 있으면 human feedback을 먼저 처리하고 QA를 재개함.

### 2.5 Provider 추상화 계층

현재 코드는 11개 프로바이더를 지원함.
...
- `providers/registry.ts`는 provider SDK를 묶어 unified registry를 만듦.
- `providers/factory.ts`는 provider별 quirks를 숨김.
    - OpenAI Codex OAuth 계정은 `.responses()` 경로를 사용함.
    - Azure는 deployment name 기반으로 `.chat()`을 호출함.
    - Ollama와 ZAI는 OpenAI-compatible adapter를 사용함.
- `resolveAuthFromQueue()`는 `providerAccounts + globalPriorityOrder`를 순회하며 계정을 선택함.
- 적절한 account를 찾지 못하면 레거시 Claude profile auth로 fallback함.
- `PhaseConfig`는 spec / planning / coding / QA 단계별 모델과 thinking level을 분리함.
...
`TASK_START` 이벤트 처리 시 실행 경로는 아래처럼 갈림.

1. task와 project를 조회함.
2. git 초기 조건을 검증함.
    - 저장소가 아니면 실행 거부
    - 초기 커밋이 없으면 실행 거부
3. 인증 상태를 검증함.
    - 활성 Claude profile auth 또는 provider account가 하나 이상 있어야 함.
4. `implementation_plan.json`의 실제 subtask 유무를 먼저 확인함.
    - in-memory `task.subtasks`보다 plan 파일이 더 신뢰 가능한 source로 취급됨.
5. 현재 XState 상태에 따라 `PLANNING_STARTED`, `PLAN_APPROVED`, `USER_RESUMED` 중 하나를 `taskStateManager`에 보냄.
6. file watcher를 시작함.
    - worktree spec dir가 이미 있으면 worktree 쪽을 우선 감시함.
7. `spec.md`가 없으면 `startSpecCreation()`을 호출함.
8. `spec.md`는 있지만 plan이 없거나 subtask가 비어 있으면 `startTaskExecution()`을 호출해 planner부터 다시 탐.
9. spec과 plan이 모두 있으면 기존 build 파이프라인을 바로 재개함.

이 흐름은 "task 카드의 상태"보다 "실제 spec/plan 파일 상태"를 우선시해 재시작 시 drift를 줄이도록 설계되어 있음.

### 3.2 build/QA 시 worktree 기본 사용

`startTaskExecution()`은 기본적으로 worktree를 사용함 (`useWorktree !== false`).
반면 spec 생성은 main project 경로의 spec dir에서 시작하고, 구현/QA 단계부터 worktree로 진입함.

- 위치: `.auto-claude/worktrees/tasks/{specId}`
- 브랜치: `auto-claude/{specId}`
- 기준 브랜치: task metadata의 `baseBranch` 또는 project 설정의 main branch

생성 과정은 멱등적으로 설계되어 있음.
...
- `pushNewBranches`가 활성화돼 있으면 새 branch를 origin에 push하고 upstream을 잡음.
- worker session의 `cwd`, `projectDir`, `specDir`는 worktree 기준으로 바뀜.
- worktree에서 실행할 때 `sourceSpecDir`는 main project spec dir를 가리켜 spec 동기화에 사용됨.

### 3.3 AI 세션 실행 상세

worker 경로의 세션 실행은 `runContinuableSession()`과 `runAgentSession()`을 중심으로 돌아감.

- 기본 max step은 500임.
- 메모리 컨텍스트가 있으면 step limit이 calibration factor에 따라 조정됨.
- `streamText()` 기반 tool loop를 사용함.
- `ProgressTracker`가 stream event를 phase/usage 정보로 변환함.
- MCP clients는 agent type별 설정에 맞춰 동적으로 붙음.

실행 중 핵심 제약:

- **컨텍스트 윈도우 관리**: 85%에서 경고, 90%에서 abort/continuation
- **수렴 유도**: `qa_reviewer`, `qa_fixer`, `spec_validation`, `pr_reviewer` 같은 agent는 step의 75% 부근에서 마무리 nudge를 받음
- **인증 갱신**: 401이면 `onAuthRefresh()` 또는 `onAccountSwitch()`를 시도함
- **계정 전환**: 429 / 401이면 priority queue에서 다음 account를 찾아 provider/model을 교체함
- **실행 결과 전달**: worker는 `task-event`, `execution-progress`, `stream-event`, `result`, `exit`를 main thread에 전달함

### 3.4 QA / human review 플로우

Aperant에는 review gate가 두 개 있음.

1. **plan review**
    - planning 완료 후 `requireReviewBeforeCoding`이 켜져 있으면 `plan_review` 상태로 들어감.
    - 사용자가 승인해야 coding으로 넘어감.
2. **final human review**
    - QA 통과 후 `human_review` 상태로 들어감.
    - `TASK_REVIEW`가 승인/반려를 결정함.
...
- 승인
    ...
    - `MARK_DONE` 이벤트 전송
- 반려
    - worktree가 있다면 main 작업공간에 반영된 merge 흔적을 `git reset`, `git checkout -- .`, `git clean -fd -e .auto-claude`로 되돌림
    - `QA_FIX_REQUEST.md`와 `feedback_images/`는 worktree spec dir가 있으면 그쪽에 기록함
    - `startQAProcess()`는 **build가 실제로 실행된 경로**를 기준으로 다시 시작함
    - `USER_RESUMED`를 보내 XState를 coding 쪽으로 되돌림

주의할 점은 "승인 = 항상 worktree 즉시 삭제"가 아니라는 점임.

- full merge handler는 성공 후 cleanupWorktree를 시도함.
- stage-only merge나 `keepWorktree` 선택 시 worktree를 남길 수 있음.
- task를 `done`으로 옮길 때도 worktree가 남아 있으면 추가 cleanup confirmation이 필요함.

### 3.5 상태기계와 phase protocol

Aperant 문서를 이해할 때 가장 빠지기 쉬운 부분이 **XState state**와 **execution phase**의 이중 구조임.
...
2. **Execution phase protocol**
    - phase: `planning`, `coding`, `rate_limit_paused`, `auth_failure_paused`, `qa_review`, `qa_fixing`, `complete`,
      `failed`
    - worker 로그 안의 `__EXEC_PHASE__:` structured marker를 우선 사용함

`AgentEvents`의 역할:

- structured progress가 있으면 그것을 그대로 사용함
- structured marker가 없을 때만 로그 문자열을 fallback으로 파싱함
- phase regression을 막음
- pause phase나 terminal phase는 fallback 텍스트로 덮어쓰지 않음

`taskStateManager`의 역할:

- worker `task-event`를 XState actor에 보냄
- legacy UI status (`in_progress`, `ai_review`, `human_review`, `done`)와 review reason을 계산함
- `xstateState`와 `executionPhase`를 main spec dir와 worktree spec dir의 `implementation_plan.json`에 함께 기록함

즉, Aperant는 "단순한 progress bar"가 아니라 **XState 상태기계 + structured phase protocol + plan file persistence**를 동시에 돌리는 구조임.
...
`Tool.define`은 모든 non-readonly 도구에 대해 공통 보안 훅을 적용함.
...
특히 spec 관련 agent는 `allowedWritePaths: [session.specDir]`가 적용되어 spec dir 밖으로 deliverable을 쓰지 못함.

### 4.3 Bash 보안 검증

현재 bash 보안 모델은 **denylist 기반 allow-by-default**임.
...
`AgentType` union 기준으로 현재 33개 agent type이 정의돼 있음.
...
과거 요약처럼 agent 수를 대략적으로 뭉뚱그릴 수도 있지만, 최신 구현을 설명하려면 **spec/PR/utilities까지 포함한 33개 agent type**이라고 보는 편이 정확함.

### 5.2 Phase별 모델 선택

`PhaseConfig`는 같은 task 안에서도 phase(spec / planning / coding / qa)별 모델과 thinking level을 다르게 줌.
...
모델 ID 접두사로 프로바이더를 자동 판별함: `claude-` → Anthropic, `gpt-`/`o1-`/`o3-` → OpenAI, `gemini-` → Google, `mistral-` → Mistral,
`llama-` → Groq, `grok-` → xAI, `glm-` → ZAI

### 5.3 Thinking Level 시스템

thinking level은 4단계로, 각 단계별 토큰 예산이 정해져 있음.
...
`claude-opus-4-6`만 `maxThinkingTokens`와 `effortLevel`을 동시에 지원함.
다른 모델은 `maxThinkingTokens`만 사용함.

CLI override(`--thinking`)가 있으면 이것이 우선함.
...
`AgentProcessManager`는 429/401을 감지하면 `auto-swap-restart-task` 이벤트를 올리고, `AgentManager.restartTask()`가 이를 받아 재시작을 수행함.
...
`taskExecutionContext.generation`이 이전 실행의 늦은 exit callback이 새 실행 컨텍스트를 청소하지 못하게 막음.
종료 후 fallback timer도 별도로 있어 "프로세스는 끝났는데 상태가 active로 남는" 경우를 완화함.
...
pause는 단순 로그 메시지가 아니라 phase protocol의 일부임.

- `rate_limit_paused`
- `auth_failure_paused`

pause 시 sentinel file이 spec dir에 기록되고, `TASK_RESUME_PAUSED`는 `RESUME` 파일을 써서 backend 재개를 신호함.

### 6.5 에러 분류 체계

`error-classifier.ts`는 8개 에러 코드로 세분화해 적절한 복구 경로를 선택함.
...
`billing_error`와 `rate_limited`의 구분이 중요함.
둘 다 429지만, "insufficient balance", "credits exhausted" 같은 키워드가 있으면 `billing_error`로 분류됨.
`billing_error`는 일시적이지 않으므로 대기 후 재시도가 의미 없음.
...
worker 프로세스 생성 시 환경변수는 다층 우선순위로 해석됨.
...
이 우선순위는 멀티 계정과 파일 기반 OAuth를 섞어 쓰는 환경에서 충돌을 줄이는 역할을 함.
...
`ai/memory/`는 **libSQL(Turso)** 위에 구축된 자체 knowledge graph 시스템임.
단순한 key-value 저장이 아니라 16종의 메모리 타입, 관계 그래프, 다채널 검색을 갖추고 있음.
...
`ai/merge/`는 멀티 task worktree에서 main으로 변경 사항을 통합할 때 사용되는 **intent-aware semantic merge** 시스템임.

**병합 파이프라인:**

1. `file-evolution.ts`가 baseline과 각 task 버전의 파일 변경 이력을 로드함.
2. `semantic-analyzer.ts`가 변경 의도를 35+ 변경 타입으로 분류함.
3. `conflict-detector.ts`가 겹치는 변경 영역을 식별함.
4. `auto-merger.ts`가 호환 가능한 변경에 대해 deterministic 전략을 적용함.
5. 모호한 충돌은 `merge_resolver` 에이전트를 호출해 AI가 해결함.
6. `orchestrator.ts`가 전체 병합 보고서를 생성함.

**변경 타입 (35+):**

import 추가/제거/수정, 함수 추가/제거/수정/이름 변경, React hook/JSX 조작, 변수/상수/클래스/메서드/속성 추가/수정, TypeScript 타입/인터페이스 추가/수정, Python
...
`ai/mcp/`는 agent type별로 필요한 MCP server를 연결함.
`mcp-handlers.ts`는 health check, server 등록, 설정 관리 같은 IPC를 제공함.

### 9.4 Roadmap / Ideation queue

`AgentQueueManager`는 roadmap과 ideation을 위한 별도 runner 계층임.
...
`terminal/`은 PTY daemon과 session persistence를 갖춘 별도 서브시스템임.
Electron main이 재시작돼도 터미널 세션을 살려 두는 방향으로 설계돼 있음.

### 9.6 외부 서비스 통합

GitHub, GitLab, Linear, changelog, insights, roadmap, ideation이 각각 독립 IPC 모듈과 러너를 가짐.
즉 Aperant는 task build app이면서 동시에 여러 AI utility feature를 품은 데스크톱 허브에 가까움.

### 9.7 크로스 플랫폼 / i18n

`platform/` 계층이 shell detection, 실행 파일 탐색, OS별 차이를 숨기고, renderer는 `react-i18next` 기반 다국어 리소스를 사용함.

---

## 10. Spec 디렉토리 구조

spec dir 내용은 complexity tier와 실행 경로에 따라 조금씩 달라지지만, 대표적인 산출물은 아래와 같음.
...
| 관점         | 장점                                                              | 트레이드오프                                                              |
|------------|-----------------------------------------------------------------|---------------------------------------------------------------------|
| 상태 일관성     | XState + plan file persistence로 UI/worker/file 상태를 맞추기 쉬움       | 상태 원천이 여러 층(XState, execution phase, plan file)이라 디버깅 진입 비용이 높음     |
| spec 파이프라인 | complexity에 따라 빠른 경로와 정밀 경로를 나눌 수 있음                            | phase 수가 많아질수록 spec 생성 흐름을 추적하기 어려움                                 |
| 개발 안전성     | worktree 격리, merge/discard 핸들러, path safety로 main 오염을 줄임       | merge 이후 cleanup은 handler와 사용자 선택에 따라 달라져 운영 규칙 이해가 필요함            |
| 확장성        | 11개 provider, phase별 모델 선택, MCP 조합으로 확장성이 높음                    | provider/account 조합이 늘수록 테스트 매트릭스가 커짐                              |
| 복원력        | auto-swap, startup recovery, pause/resume, stale exit guard가 있음 | 상태 복원 로직이 복잡해 edge case 검증 부담이 커짐                                  |
| 도구 보안      | denylist, validator, write-path containment이 결합돼 있음             | allow-by-default bash 모델이라 새 위험 패턴이 나오면 validator/denylist 갱신이 필요함 |
...
1. **상태를 파일만으로 보지 않음.** `taskStateManager`, phase protocol, plan persistence가 함께 돌아감.
2. **spec과 build의 흐름이 다름.** spec은 complexity-first, build는 planning/coding/QA 중심임.
3. **worktree는 단순 생성으로 끝나지 않음.** merge, reject, discard, cleanup confirmation까지 후속 핸들러가 이어짐.

즉 Aperant는 "코드를 자동으로 써 주는 데스크톱 앱"이라기보다,
에이전트 실행을 실제 운영 가능한 형태로 만들기 위해 상태기계, review gate, worktree, provider queue를 함께 묶은 실행 시스템에 가까움.
